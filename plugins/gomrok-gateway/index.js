...
// URL of the page we want to scrape
        const url = "https://lamininbrand.com/api.php?method=update_prices";
...
// Async function which scrapes the data
        async function scrapeTheData() {
            try {
                ...
                fs.writeFile("./public_media/customer/rates.json", JSON.stringify(countries, null, 2), (err) => {
                    if (err) {
                        console.error(err);
                        return;
                    }
                            ...
                            async function scrapeData() {
                                try {
                                    ...
                                    const {data, headers} = await axios({
                                        method: 'get',
                                        url,
                                        headers: {'Cookie': 'JSESSIONID=978A0983345EF4C8C0AAAB533956FA3F.e'}
                                    });
                                    // Load HTML we fetched in the previous line
                                    const $ = cheerio.load(data);
                                    // Select all the list items in plainlist class
